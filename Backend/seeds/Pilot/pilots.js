...
const seedPilots = async () => {
  try {
    ...
    // Get all languages for association
    const allLanguages = await Language.findAll();
    const languageByName = allLanguages.reduce((acc, lang) => {
      acc[lang.name] = lang;
      return acc;
    }, {});
    ...
    const getRandomLanguagesForSeniority = (seniorityLevel) => {
      const languages = Object.values(languageByName);
      let count;
      ...
      return shuffled(languages).slice(0, count);
    };
