...
export async function dropFlightRecipesTable() {
  try {
    await sql`DROP TABLE IF EXISTS flight_recipes CASCADE`;
    console.log('🗑️  `flight_recipes` table dropped');
    return true;
  } catch (error) {
    console.error('❌ Error dropping flight_recipes table:', error);
    throw error;
  }
}