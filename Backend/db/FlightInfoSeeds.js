/**
 * FlightInfoSeeds.js
 * Handles seeding of flight-related data:
 * - Airport data
 * - Vehicle types
 * - Sample flights
 */
...
/**
 * Seed airports data
 * @returns {Promise<Object>} Map of airport codes to IDs
 */
export async function seedAirports() {
  try {
    ...
    // Return airport map for use in flight seeding
 ...
 * @returns {Promise<Array>} Array of vehicle type IDs
 */
export async function seedVehicleTypes() {
  try {
    ...
    // Return vehicle type IDs for use in flight seeding
 ...
 * @param {Array} vehicleTypeIds - Array of vehicle type IDs from seedVehicleTypes
 * @param {Object} airportMap - Map of airport codes to IDs from seedAirports
 */
export async function seedFlights(vehicleTypeIds, airportMap) {
  try {
...
/**
 * Seed all flight-related data (airports, vehicle types, and flights)
 * Call this function to populate the database with initial data
 */
export async function seedAllFlightData() {
  try {
    console.log("🌱 Starting flight data seeding...\n");

    // Seed in order of dependencies
    const airportMap = await seedAirports();
    const vehicleTypeIds = await seedVehicleTypes();
    await seedFlights(vehicleTypeIds, airportMap);

    console.log("\n✅ Flight data seeding completed successfully!");
    return true;
  } catch (error) {
    console.error("\n❌ Error seeding flight data:", error);
    throw error;
  }
}
